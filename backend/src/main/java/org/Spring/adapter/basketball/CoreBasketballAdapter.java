package org.Spring.adapter.basketball;
...
/**
 * Adapter for the ESPN basketball scoreboard API.
 *
 * Status mapping:
 *   state=in  + HALFTIME   -> "HT"
 *   state=in  + period 1   -> "Q1"
 *   state=in  + period 2   -> "Q2"
 *   state=in  + period 3   -> "Q3"
 *   state=in  + period 4   -> "Q4"
 *   state=in  + period 5+  -> "OT"
 *   state=post             -> "FT" / "FT-OT" / "Canceled" / "Postponed"
 *   state=pre              -> "Scheduled" / "TBD"
 */
public class CoreBasketballAdapter {
    ...
    /**
     * Maps ESPN state/name/period to the app's status vocabulary.
     * Q1-Q4 and OT give the fetcher and frontend richer live context.
     */
    private String mapStatus(String state, String name, int period) {
        if ("in".equals(state)) {
            if (name.contains("HALFTIME")) return "HT";
            // Overtime: ESPN uses period 5+ for OT periods
