...
import { getStatistics, getLeaders } from "@/lib/api/espn";
...
export default async function StatisticsPage({ searchParams }: Props) {
  ...
  // Parse stat categories from raw
  const categories = statsRaw?.leaders ?? statsRaw?.categories ?? [];
  const hasCats = Array.isArray(categories) && categories.length > 0;

  return (
    <div className="container" style={{ paddingTop: 28, paddingBottom: 48 }}>
      <Link href={`/football?league=${league}`} style={{ fontSize: 13, fontWeight: 500, color: "var(--text-secondary)", textDecoration: "none", display: "inline-block", marginBottom: 20 }}>
        ← {leagueName(league)}
      </Link>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 24 }}>
        {LEAGUES.map(l => (
          <Link key={l.slug} href={`/football/statistics?league=${l.slug}`} className="pill" style={{ textDecoration: "none", fontWeight: l.slug === league ? 700 : 400, opacity: l.slug === league ? 1 : 0.7 }}>
            {l.short}
          </Link>
        ))}
      </div>
      ...
      {leaders.length > 0 && (
        <section style={{ marginBottom: 32 }}>
          <h2 style={{ fontSize: 12, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 14 }}>Top Scorers & Leaders</h2>
      ...
      {/* Raw stat categories from /statistics endpoint */}
      {hasCats && (
        <section>
      ...
      {!hasCats && leaders.length === 0 && (
        <p style={{ fontSize: 14, color: "var(--text-muted)", textAlign: "center", padding: 40 }}>
          No statistics available for {leagueName(league)}.
        </p>
      )}
    </div>
  );
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function groupBy<T extends Record<string, any>>(arr: T[], key: string): [string, T[]][] {
