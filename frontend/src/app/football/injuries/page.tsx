...
import { InjuryList } from "@/components/football/TeamDetailCards";
...
export default async function InjuriesPage({ searchParams }: Props) {
  ...
  const injuries = await getInjuries(league);
  ...
  return (
    <div className="container" style={{ paddingTop: 28, paddingBottom: 48 }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 24 }}>
        {LEAGUES.map(l => (
          <Link key={l.slug} href={`/football/injuries?league=${l.slug}`} className="pill" style={{ textDecoration: "none", fontWeight: l.slug === league ? 700 : 400, opacity: l.slug === league ? 1 : 0.7 }}>
            {l.short}
          </Link>
        ))}
      </div>
