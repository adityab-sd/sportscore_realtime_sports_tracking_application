...
// This is the single source of truth — no dependency on URL ?team param.
...
// ─── Jersey SVG ───────────────────────────────────────────────────────────────
function JerseyHero({ jersey, name, teamLogo }: {
  ...
  return (
    <svg viewBox="0 0 500.152 500.152" xmlns="http://www.w3.org/2000/svg"
      style={{ width: 180, height: 180, display: "block", filter: "drop-shadow(0 8px 24px rgba(0,0,0,0.3))" }}>
      <defs>
        <clipPath id="jClip">
          <path d="M463.776,116.236c-9.688-30.68-32.52-54.76-62.656-66.056l-79.048-29.648V2.076h-144v18.456l-79.048,29.64 c-30.136,11.304-52.968,35.376-62.656,66.056L0,231.412l95.44,28.632l18.632-62.128v300.16h272v-300.16l18.64,62.12l95.44-28.632 L463.776,116.236z" />
        </clipPath>
      </defs>
      <path
      ...
      />
      <path
        d="M463.776,116.236c-9.688-30.68-32.52-54.76-62.656-66.056l-79.048-29.648V2.076h-144v18.456l-79.048,29.64 c-30.136,11.304-52.968,35.376-62.656,66.056L0,231.412l95.44,28.632l18.632-62.128"
        fill="none" stroke="rgba(255,255,255,0.12)" strokeWidth="2"
      />
      {/* {teamLogo && (
        <image href={teamLogo} x="170" y="80" width="160" height="160" opacity="0.15" clipPath="url(#jClip)" />
      )} */}
...
export default async function PlayerPage({ params, searchParams }: Props) {
  ...
  // Note: ?team param is intentionally ignored — club resolved from Core API

  // Resolve player's actual club (always, regardless of URL context)
  const [clubResult, overviewRaw] = await Promise.all([
    resolvePlayerClub(id),
    getAthleteOverviewRaw(contextLeague, id),
  ]);

  // Must have a club to render anything meaningful
  if (!clubResult) return notFound();

  const { team: brandTeam, leagueSlug: clubLeagueSlug, teamId: clubTeamId, roster } = clubResult;

// Find this player in their actual club roster (may not be present due to ID mismatches)
  const rosterPlayer = roster.find(p => p.id === id);

  // Build player info from Core API if not in roster
  const player: ESPNPlayer = rosterPlayer ?? {
    id,
    name: brandTeam.name, // fallback — will be overwritten below
    jersey: null,
    position: null,
    age: null,
    nationality: null,
  ...
  // Enrich from Core API data (always more reliable than roster for the player's own info)
    ...
    <div style={{ background: "var(--cloud)", minHeight: "100vh" }}>
      ...
      <div style={{ background: bannerColor, borderBottom: "1px solid rgba(255,255,255,0.08)", overflow: "hidden", position: "relative" }}>
        {brandTeam?.logo && (
          <div style={{
            ...
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={brandTeam.logo} alt="" width={260} height={260} style={{ width: 260, height: 260, objectFit: "contain" }} />
          </div>
        )}
        ...
        <div className="container" style={{ paddingTop: 0, paddingBottom: 0, position: "relative", zIndex: 1 }}>
          <div style={{ display: "flex", alignItems: "flex-end", gap: 28, flexWrap: "wrap" }}>
            <div style={{ flexShrink: 0, paddingTop: 24, marginBottom: -4 }}>
              {headshot ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={headshot} alt={player.name}
                  style={{ width: 180, height: 180, objectFit: "cover", objectPosition: "top", display: "block" }} />
              ) : (
            ...
            <div style={{ flex: 1, minWidth: 240, paddingBottom: 28, paddingTop: 32 }}>
              ...
              <div style={{ marginBottom: 14 }}>
                {firstName && (
                  <div style={{ fontSize: "clamp(16px, 2.5vw, 22px)", fontWeight: 300, color: "rgba(255,255,255,0.8)", lineHeight: 1.1 }}>
                    {firstName.toUpperCase()}
                  </div>
                )}
                <div style={{ fontSize: "clamp(28px, 5vw, 52px)", fontWeight: 900, color: "#fff", lineHeight: 1, letterSpacing: "-1.5px" }}>
      ...
      <div className="container" style={{ paddingTop: 24, paddingBottom: 48 }}>
        <div style={{ display: "grid", gridTemplateColumns: "260px 1fr 260px", gap: 20, alignItems: "start" }}>
          ...
          {/* LEFT: Switch Player — uses club roster, links without ?team */}
          <PlayerSidebars
