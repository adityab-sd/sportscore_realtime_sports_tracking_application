...
import LineupsCard from "@/components/football/LineupsCard";
...
function EventIcon({ type, detail }: { type: string; detail: string }) {
  ...
  if (type === "goal" || d.includes("goal")) return (
    <svg width="14" height="14" viewBox="0 0 490 490" fill="var(--obsidian)">
...
export default async function MatchPage({ params, searchParams }: Props) {
  ...
  return (
    <div className="container" style={{ maxWidth: 720, paddingTop: 24, paddingBottom: 40 }}>
      ...
      <LineupsCard lineups={match.lineups ?? []} homeTeam={match.homeTeam} awayTeam={match.awayTeam} league={league} />
      ...
      {!isLive && match.events.length === 0 && match.statusState !== "pre" && (
