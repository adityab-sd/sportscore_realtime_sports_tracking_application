...
import { getRoster, getTeam } from "@/lib/api/basketball";
...
export default async function PlayerPage({ params, searchParams }: Props) {
  ...
  return (
    <div className="container" style={{ maxWidth: 620, paddingTop: 28, paddingBottom: 40 }}>
      <Link href={`/basketball/team/${teamId}?league=${league}`} style={{ fontSize: 13, fontWeight: 500, color: "var(--text-secondary)", textDecoration: "none", display: "inline-block", marginBottom: 24 }}>← {team?.name ?? "Team"}</Link>
