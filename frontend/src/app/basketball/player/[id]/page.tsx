...
import PlayerSidebars from "@/components/basketball/PlayerSidebars";
import PlayerNewsFeed from "@/components/football/PlayerNewsFeed";
import TeamLogo from "@/components/basketball/TeamLogo";

export const dynamic = "force-dynamic";

const SPORT = "basketball";
const HEADSHOT_LEAGUE = "nba";
              ...
              return (
                <tr key={eventId || i} style={{ borderBottom: "1px solid var(--border)" }}>
                  <td style={{ padding: "7px 12px" }}>
                    {eventId
                      ? <Link href={`/basketball/${eventId}?league=${league}`} style={{ textDecoration: "none", display: "block" }}>{cell}</Link>
                      : cell}
                  </td>
    ...
    <div style={{ background: "var(--white)", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden" }}>
      ...
      {eventId && (
        <div style={{ borderTop: "1px solid var(--border)", padding: "10px 16px", textAlign: "center" }}>
          <Link href={`/basketball/${eventId}?league=${league}`} style={{ fontSize: 12, fontWeight: 600, color: "var(--navy)", textDecoration: "none" }}>Game Details →</Link>
        </div>
      )}
    </div>
  );
...
export default async function PlayerPage({ params, searchParams }: Props) {
  ...
  const nationality = bio.citizenship ?? bio.birthPlace?.country ?? bio.displayHeight ?? null;
      ...
      <div className="container" style={{ paddingTop: 24, paddingBottom: 48 }}>
        <div style={{ display: "grid", gridTemplateColumns: "260px 1fr 260px", gap: 20, alignItems: "start" }}>
          ...
          <PlayerSidebars
            ...
            quickLinks={teamId ? [
              { label: "Roster", href: `/basketball/team/${teamId}?league=${league}` },
              { label: "Schedule", href: `/basketball/team/${teamId}?league=${league}` },
              { label: "News", href: `/basketball/news?league=${league}` },
            ] : []}
            leagueQuickLinks={leagueInfo ? [
              { label: "Live Scores", href: `/baseball?league=${league}` },
              { label: "Standings", href: `/basketball/standings?league=${league}` },
              { label: "Schedule", href: `/basketball/fixtures?league=${league}` },
              { label: "Injuries", href: `/basketball/injuries?league=${league}` },
              { label: "News", href: `/basketball/news?league=${league}` },
            ] : []}
