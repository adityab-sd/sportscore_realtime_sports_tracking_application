    ...
    <div style={{ minHeight: "100vh", background: "var(--white)" }}>
      <div style={{ position: "relative", width: "100%", aspectRatio: "16/9", overflow: "hidden", background: "var(--obsidian)" }}>
        {article.image && (
          ...
          <img src={article.image} alt={article.headline} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", objectPosition: "center top" }} />
        )}
        ...
        <div style={{ position: "absolute", top: 0, left: 0, right: 0, zIndex: 10 }}>
          <div className="container" style={{ height: 48, display: "flex", alignItems: "center", gap: 16 }}>
            <Link href="/basketball/news" style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: 600, color: "rgba(255,255,255,0.9)", textDecoration: "none", background: "rgba(0,0,0,0.25)", padding: "5px 12px", borderRadius: 20, backdropFilter: "blur(8px)", border: "1px solid rgba(255,255,255,0.15)" }}>
        ...
        <div style={{ position: "absolute", bottom: "4%", left: 0, right: 0, zIndex: 10 }}>
          <div className="container" style={{ maxWidth: 760 }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(0,0,0,0.35)", backdropFilter: "blur(6px)", padding: "5px 12px 5px 5px", borderRadius: 20 }}>
              <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.6px", textTransform: "uppercase", color: "#fff", background: "var(--blue)", padding: "4px 10px", borderRadius: 16 }}>{article.category}</span>
              <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 12, color: "rgba(255,255,255,0.85)", fontWeight: 500 }}><Clock size={11} /><span suppressHydrationWarning>{timeAgo(article.published)}</span></span>
            </div>
          </div>
        </div>
      </div>
      <div className="container" style={{ maxWidth: 760, paddingTop: 28, paddingBottom: 16 }}>
        <h1 style={{ fontSize: "clamp(26px, 4vw, 40px)", fontWeight: 800, color: "var(--obsidian)", lineHeight: 1.15, letterSpacing: "-0.7px", margin: "0 0 24px" }}>{article.headline}</h1>
        {article.description && (
          <div style={{ borderLeft: "3px solid var(--blue)", paddingLeft: 20, marginBottom: 36 }}>
            <p style={{ fontSize: 18, color: "var(--text-secondary)", lineHeight: 1.75, margin: 0, fontStyle: "italic" }}>{article.description}</p>
          </div>
        )}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12, paddingTop: 20, borderTop: "1px solid var(--border)", marginBottom: 56 }}>
          ...
          {article.link && (
            <a href={article.link} target="_blank" rel="noopener noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: 600, color: "var(--navy)", background: "var(--navy-light)", padding: "8px 14px", borderRadius: 8, textDecoration: "none" }}>
              <ExternalLink size={13} />Full article on ESPN
            </a>
          )}
        </div>
      </div>
      {related.length > 0 && (
        <div style={{ borderTop: "1px solid var(--border)", background: "var(--cloud)", paddingTop: 40, paddingBottom: 56 }}>
          <div className="container">
            ...
            <div className="news-grid">{related.map(a => <NewsCard key={a.id} article={a} sport="basketball" />)}</div>
          </div>
        </div>
      )}
    </div>
