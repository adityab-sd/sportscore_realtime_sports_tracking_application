...
function parseRankings(data: RawJSON): { polls: { name: string; ranks: { rank: number; team: string; shortName: string; logo: string | null; record: string; points: number; trend: string }[] }[] } {
  ...
  // ADDRESSED: rank arrays assumed valid: (poll.ranks ?? poll.entries ?? []) can be a non-array response shape, so .map may throw. EXAMPLE: const entries = Array.isArray(poll.ranks ?? poll.entries) ? (poll.ranks ?? poll.entries) : [];
  return {
...
export default async function RankingsPage({ searchParams }: Props) {
  ...
  // ADDRESSED: validate league query before fetching: any string is accepted, but the UI only offers men's/women's CBB. EXAMPLE: const safeLeague = league === "womens-college-basketball" ? league : "mens-college-basketball";
    ...
    <div className="container" style={{ paddingTop: 28, paddingBottom: 40 }}>
      ...
      {polls.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 28 }}>
          {/* ADDRESSED: index keys for polls/ranks: ranking lists reorder weekly, so key={pi}/key={ri} can preserve the wrong row. EXAMPLE: {polls.map((poll) => <section key={poll.name}>...</section>)} and {poll.ranks.map((r) => <div key={`${poll.name}:${r.rank}:${r.team}`}>...</div>)}. */}
          {polls.map((poll, pi) => (
            <section key={pi}>
              ...
              <div style={{ background: "var(--white)", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden" }}>
                ...
                {poll.ranks.map((r, ri) => (
                  <div key={ri} style={{ display: "grid", gridTemplateColumns: "40px 1fr 80px 60px", padding: "10px 16px", borderBottom: ri < poll.ranks.length - 1 ? "1px solid var(--border)" : "none", alignItems: "center" }}>
                    ...
                    {/* ADDRESSED: zero points display as missing: || treats legitimate 0 as "–". EXAMPLE: {r.points != null ? r.points : "–"}. */}
                    <span className="stat-num" style={{ textAlign: "center", fontSize: 12, color: "var(--text-secondary)" }}>{r.points != null ? r.points : "–"}</span>
                  </div>
                ))}
              </div>
            </section>
