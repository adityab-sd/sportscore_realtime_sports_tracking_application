...
function groupByConference(rows: BBStandingRow[]): { name: string; rows: BBStandingRow[] }[] {
  ...
  for (const r of rows) {
    ...
    // ADDRESSED: avoid non-null assertion in aggregation: (buckets.get(key) ?? []) assumes the prior mutation always succeeded and hides future refactor bugs. EXAMPLE: const bucket = buckets.get(key) ?? []; bucket.push(r); buckets.set(key, bucket);
    (buckets.get(key) ?? []).push(r);
  }
  return Array.from(buckets.entries())
    // ADDRESSED: rank sort can produce NaN: missing ranks from ESPN make a.rank - b.rank unstable. EXAMPLE: rs.sort((a, b) => (a.rank ?? Number.MAX_SAFE_INTEGER) - (b.rank ?? Number.MAX_SAFE_INTEGER));
...
export default async function StandingsPage({ searchParams }: Props) {
  ...
  // ADDRESSED: validate league query before fetching: any URL value is forwarded to getStandings and rendered in links. EXAMPLE: const safeLeague = LEAGUES.some((l) => l.slug === league) ? league : "nba";
