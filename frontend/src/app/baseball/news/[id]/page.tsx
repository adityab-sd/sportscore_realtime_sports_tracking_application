      ...
      <div style={{ position: "relative", width: "100%", aspectRatio: "16/9", overflow: "hidden", background: "var(--obsidian)" }}>
        ...
        <div style={{ position: "absolute", top: 0, left: 0, right: 0, zIndex: 10 }}>
          <div className="container" style={{ height: 48, display: "flex", alignItems: "center", gap: 16 }}>
            <Link href="/baseball/news" style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: 600, color: "rgba(255,255,255,0.9)", textDecoration: "none", background: "rgba(0,0,0,0.25)", padding: "5px 12px", borderRadius: 20, backdropFilter: "blur(8px)", border: "1px solid rgba(255,255,255,0.15)" }}>
              <ArrowLeft size={14} />
              Back to News
            </Link>
          </div>
      ...
      <div className="container" style={{ maxWidth: 760, paddingTop: 28, paddingBottom: 16 }}>
        ...
        {article.description && (
          <div style={{ borderLeft: "3px solid var(--blue)", paddingLeft: 20, marginBottom: 36 }}>
            <p style={{ fontSize: 18, color: "var(--text-secondary)", lineHeight: 1.75, margin: 0, fontStyle: "italic" }}>
              {article.description}
            </p>
          </div>
        )}
        ...
        <div style={{ borderTop: "1px solid var(--border)", background: "var(--cloud)", paddingTop: 40, paddingBottom: 56 }}>
          <div className="container">
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
              ...
              <Link href="/baseball/news" style={{ fontSize: 13, fontWeight: 600, color: "var(--navy)", textDecoration: "none" }}>All news →</Link>
            </div>
            <div className="news-grid">
              {related.map(a => <NewsCard key={a.id} article={a} sport="baseball" />)}
            </div>
          </div>
        </div>
      )}
