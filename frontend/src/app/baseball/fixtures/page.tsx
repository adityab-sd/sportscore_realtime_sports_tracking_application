...
import { getFixtures } from "@/lib/api/baseball";
import { LEAGUES, leagueName } from "@/types/baseball";
import FixtureCard from "@/components/baseball/FixtureCard";
  ...
  return (
    <div className="container" style={{ paddingTop: 28, paddingBottom: 48 }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 24, gap: 12, flexWrap: "wrap" }}>
        ...
        <Link href={`/baseball/league/${league}`} style={{ fontSize: 13, fontWeight: 600, color: "var(--navy)", background: "var(--navy-light)", padding: "8px 14px", borderRadius: 8, textDecoration: "none" }}>
          ← League home
        </Link>
      </div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 24, overflowX: "auto", paddingBottom: 4 }}>
        {LEAGUES.map(l => (
          <Link key={l.slug} href={`/baseball/fixtures?league=${l.slug}`} className={`pill${league === l.slug ? " active" : ""}`} style={{ textDecoration: "none" }}>
            {l.short}
          </Link>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 28 }} className="page-split">
        ...
        <section>
          ...
          {upcoming.length === 0 ? (
            <p style={{ fontSize: 13, color: "var(--text-muted)" }}>No upcoming games in the next window.</p>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
