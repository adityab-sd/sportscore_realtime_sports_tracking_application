...
import { getScoreboard, getFixtures, getNews, BBGame } from "@/lib/api/baseball";
import { LEAGUES } from "@/types/baseball";
import LiveBaseball from "@/components/baseball/LiveBaseball";

export const metadata: Metadata = {
  title: "Baseball — SportScore",
  description: "Live MLB & NCAA baseball scores, standings, and news.",
};
  ...
  return (
    <div className="container" style={{ paddingTop: 28, paddingBottom: 40 }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 24, gap: 12, flexWrap: "wrap" }}>
        <div>
          <h1 style={{ fontSize: "clamp(22px,4vw,28px)", fontWeight: 800, color: "var(--obsidian)", margin: "0 0 4px", letterSpacing: "-0.5px" }}>Baseball</h1>
          <p style={{ fontSize: 13, color: "var(--text-muted)", margin: 0 }}>Live scores, results &amp; upcoming games</p>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <Link href="/baseball/news" style={{ fontSize: 13, fontWeight: 600, color: "var(--navy)", background: "var(--navy-light)", padding: "8px 14px", borderRadius: 8, textDecoration: "none" }}>News</Link>
        ...
        <aside className="sport-layout__sidebar">
          <div style={{ background: "var(--white)", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden", position: "sticky", top: 80 }}>
            ...
            <div style={{ padding: "10px 16px", borderTop: "1px solid var(--border)", textAlign: "center" }}>
              <Link href="/baseball/news" style={{ fontSize: 12, fontWeight: 600, color: "var(--navy)", textDecoration: "none" }}>View all news →</Link>
            </div>
          </div>
        </aside>
      </div>
