...
import { getTransactions } from "@/lib/api/baseball";
  ...
  return (
    <div className="container" style={{ paddingTop: 28, paddingBottom: 48 }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 24 }}>
        {LEAGUES.map(l => (
          <Link key={l.slug} href={`/baseball/transactions?league=${l.slug}`} className="pill" style={{ textDecoration: "none", fontWeight: l.slug === league ? 700 : 400, opacity: l.slug === league ? 1 : 0.7 }}>
            {l.short}
          </Link>
        ))}
      </div>
      ...
      {transactions.length > 0 ? (
        ...
        <p style={{ fontSize: 14, color: "var(--text-muted)", textAlign: "center", padding: 40 }}>
          No transactions available for {leagueName(league)}.
        </p>
      )}
    </div>
  );
