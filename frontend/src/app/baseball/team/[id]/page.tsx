...
import { getTeam, getRoster, getStandings, getNews, BBGame, BBNews } from "@/lib/api/baseball";
import { LEAGUES } from "@/types/baseball";
import TeamPageClient from "@/components/baseball/TeamPageClient";
...
// ─── Team schedule (direct ESPN baseball, season-aware) ─────────────────────
// The backend exposes /teams/{id}/schedule as raw passthrough; we call ESPN
// directly here to parse it into the typed BBGame shape the client renders.
/* eslint-disable @typescript-eslint/no-explicit-any */
async function fetchTeamSchedule(league: string, teamId: string, season: string): Promise<BBGame[]> {
  try {
    const res = await fetch(
      `https://site.web.api.espn.com/apis/site/v2/sports/baseball/${league}/teams/${teamId}/schedule?season=${season}`,
    ...
    return events.map((event: any): BBGame => {
      ...
      return {
        ...
        inningDetail: status?.type?.detail ?? null,
        homePitcher: null,
        awayPitcher: null,
      };
    });
...
// ─── Team stats + leaders (direct ESPN core, baseball) ──────────────────────
async function fetchTeamStats(league: string, teamId: string, season: string) {
  const base = `https://sports.core.api.espn.com/v2/sports/baseball/leagues/${league}/seasons/${season}/types/2`;
...
function parseTeamLeaders(raw: any): { category: string; name: string; value: string; athleteId: string; headshot: string | null }[] {
  ...
  for (const cat of cats) {
    ...
    result.push({
      ...
      athleteId,
      headshot: athleteId ? `https://a.espncdn.com/i/headshots/mlb/players/full/${athleteId}.png` : null,
    });
  }
  return result;
}
...
export default async function TeamPage({ params, searchParams }: Props) {
  ...
  const { league = "mlb", season } = await searchParams;
