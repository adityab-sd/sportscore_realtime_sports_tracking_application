...
import {
  CAL_WEEKDAYS, MONTH_NAMES,
  startOfDayLocal, addDays, toKey, pillLabel, friendlyLabel, isSameDayDates,
} from "@/lib/dates";
...
/**
 * Date navigator: a sliding pill strip on desktop, a compact bar + popover
 * calendar on mobile. Sport-agnostic — used by every Live section and league
 * page. `selected` is always a local start-of-day Date.
 */
export default function DatePicker({ selected, onSelect }: { selected: Date; onSelect: (d: Date) => void }) {
  ...
  // Close the mobile calendar when the viewport grows past the mobile breakpoint.
  useEffect(() => {
    const mq = window.matchMedia("(max-width: 720px)");
    const handler = (e: MediaQueryListEvent) => { if (!e.matches) setCalOpen(false); };
  ...
  const monthLabel = useMemo(() => {
    const u = [...new Set(pills.map(p => p.toLocaleDateString("en-US", { month: "long", year: "numeric" })))];
  ...
  // Calendar grid (Mon-first), padded with neighbouring-month days.
    ...
    <div style={{ marginBottom: 20 }}>
      ...
      <div className="dp-desktop">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, justifyContent: "center", overflow: "hidden" }}>
          <button onClick={() => shift("left")} aria-label="Previous week" style={{ background: "none", border: "none", cursor: "pointer", fontSize: 20, color: "var(--text-secondary)", padding: "0 4px", flexShrink: 0, lineHeight: 1 }}>‹</button>
          <div style={{ display: "flex", gap: 6, transition: "transform 0.18s ease, opacity 0.18s ease", transform: slideDir === "right" ? "translateX(-20px)" : slideDir === "left" ? "translateX(20px)" : "translateX(0)", opacity: slideDir ? 0 : 1 }}>
            {pills.map(d => {
              ...
              return (
                <button key={key} onClick={() => onSelect(d)} style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", width: 72, height: 52, borderRadius: 8, cursor: "pointer", flexShrink: 0, border: isSel ? "2px solid var(--navy)" : "1px solid var(--border)", background: isSel ? "var(--navy)" : "var(--white)", color: isSel ? "#fff" : isWE ? "var(--text-secondary)" : "var(--obsidian)", transition: "all 0.15s ease", transform: isSel ? "translateY(-1px)" : "translateY(0)", boxShadow: isSel ? "0 4px 12px rgba(30,58,138,0.25)" : "none" }}>
          ...
          <button onClick={() => shift("right")} aria-label="Next week" style={{ background: "none", border: "none", cursor: "pointer", fontSize: 20, color: "var(--text-secondary)", padding: "0 4px", flexShrink: 0, lineHeight: 1 }}>›</button>
        </div>
      </div>
      ...
      <div className="dp-mobile" ref={calRef} style={{ position: "relative" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 0, background: "var(--white)", border: "1px solid var(--border)", borderRadius: 10, overflow: "hidden", userSelect: "none" }}>
          <button onClick={prevDay} aria-label="Previous day" style={mobileNavBtn}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6" /></svg></button>
          <button onClick={() => setCalOpen(o => !o)} style={{ ...mobileNavBtn, flex: 1, minWidth: 130, fontSize: 14, fontWeight: 700, gap: 6, color: "var(--obsidian)", justifyContent: "center" }} suppressHydrationWarning>
          ...
          <button onClick={nextDay} aria-label="Next day" style={mobileNavBtn}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6" /></svg></button>
        </div>
        {calOpen && (
          <div style={{ position: "absolute", top: "calc(100% + 6px)", left: 0, right: 0, zIndex: 30, background: "var(--white)", border: "1px solid var(--border)", borderRadius: 14, boxShadow: "0 12px 36px rgba(0,0,0,0.14)", padding: "16px 14px 12px", animation: "dp-cal-in 0.15s ease" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
              ...
              <div style={{ display: "flex", gap: 4 }}>
                <button onClick={prevMonth} aria-label="Previous month" style={calArrowBtn}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6" /></svg></button>
                <button onClick={nextMonth} aria-label="Next month" style={calArrowBtn}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6" /></svg></button>
              </div>
            </div>
