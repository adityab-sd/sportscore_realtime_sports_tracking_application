...
// ─────────────────────────────────────────────
// Athlete Gamelog (Last 5 Matches style)
// ─────────────────────────────────────────────
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 640, fontSize: 12 }}>
          ...
          <tbody>
            {rows.map((row: RawJSON, ri: number) => {
              ...
              const comp = eventInfo?.leagueName ?? eventInfo?.seasonType ?? "";
              const t = Date.parse(String(eventInfo?.gameDate ?? ""));
              const dateStr = Number.isFinite(t)
                ? new Date(t).toLocaleDateString("en-US", { timeZone: "UTC", month: "short", day: "numeric" })
                : "";

              return (
                <tr key={eventId || ri} style={{ borderBottom: "1px solid var(--border)" }}>
                  <td style={{ padding: "7px 12px", position: "sticky", left: 0, background: "var(--white)" }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: "var(--obsidian)" }}>
                      {opponent ? `vs ${opponent}` : `Match ${ri + 1}`}
                    </div>
                    <div style={{ fontSize: 10, color: "var(--text-muted)" }}>
                      {[dateStr, result, comp].filter(Boolean).join(" · ")}
                    </div>
                  </td>
