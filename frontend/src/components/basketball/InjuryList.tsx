...
function statusColor(status: string): string {
  ...
  if (s.includes("out")) return "#dc2626";
  if (s.includes("day") || s.includes("question") || s.includes("doubt")) return "#d97706";
    ...
    <div style={{ background: "var(--white)", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden" }}>
      {injuries.map((inj, i) => (
        <div key={`${inj.athleteId ?? inj.athleteName}-${i}`} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", borderBottom: i < injuries.length - 1 ? "1px solid var(--border)" : "none" }}>
          ...
          <span style={{ fontSize: 10, fontWeight: 700, color: statusColor(inj.status), textTransform: "uppercase", letterSpacing: "0.4px", flexShrink: 0 }}>{inj.status}</span>
        </div>
      ))}
    </div>
  );
