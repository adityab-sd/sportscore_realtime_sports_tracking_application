...
function statusColor(s: string) {
  // ADDRESSED: status assumed present: (s ?? "").toLowerCase() throws if an injury status is null from the feed. EXAMPLE: const l = (s ?? "").toLowerCase();
...
export default function InjuryList({ injuries, showTeam = false }: { injuries: BBInjury[]; showTeam?: boolean }) {
  // ADDRESSED: array prop assumed non-null: (injuries ?? []).length crashes if team details omit injuries. EXAMPLE: const safeInjuries = Array.isArray(injuries) ? injuries : [];
