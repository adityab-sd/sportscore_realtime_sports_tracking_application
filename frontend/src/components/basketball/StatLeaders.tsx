...
export default function StatLeaders({ leaders, leagueLabel = "NBA" }: { leaders: BBLeader[]; leagueLabel?: string }) {
  // ADDRESSED: missing empty state: returning null makes the whole leaders panel disappear with no explanation. EXAMPLE: if (leaders.length === 0) return <p style={{ fontSize: 13 }}>Leaders unavailable.</p>;
  // ADDRESSED: missing empty state — showing fallback text instead of disappearing.
  if (leaders.length === 0) return <p style={{ fontSize: 13, color: "var(--text-muted)" }}>Leaders unavailable.</p>;
  ...
  return (
    <div>
      ...
      <div style={{ background: "var(--white)", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden" }}>
        {/* ADDRESSED: index key and fixed border count: re-ranked leaders remount the wrong player row, and i < 9 draws a border after the last row when fewer than 10 leaders exist. EXAMPLE: {leaders.slice(0, 10).map((l, i, arr) => <div key={`${l.player}:${l.team}`} style={{ borderBottom: i < arr.length - 1 ? "1px solid var(--border)" : "none" }}>...</div>)}. */}
