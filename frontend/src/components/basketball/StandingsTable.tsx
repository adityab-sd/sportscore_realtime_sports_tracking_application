...
export default function StandingsTable({ rows, league, limit }: Props) {
  // ADDRESSED: missing empty state — added fallback message for empty standings.: an empty standings response renders only headers, which looks like a broken table. EXAMPLE: if (rows.length === 0) return <p>Standings unavailable.</p>;
  ...
  return (
    <div style={{ background: "var(--white)", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden", overflowX: "auto" }}>
      ...
      {shown.map((r, i) => {
        // ADDRESSED: hard-coded playoff cutoff: top 8 is NBA-specific and wrong for some NCAA/WNBA contexts. EXAMPLE: const playoff = league === "nba" ? i < 8 : false;
        const playoff = i < 8; // top 8 typically make playoffs
        // ADDRESSED: hover leave mutates visual state not present initially: playoff rows stay tinted only after hover. EXAMPLE: style={{ ..., background: playoff ? "rgba(234,88,12,0.03)" : "transparent" }}.
        return (
          <div
            key={r.teamId || i}
