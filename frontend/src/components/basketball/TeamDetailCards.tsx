...
function parseSchedule(data: RawJSON): ParsedScheduleGame[] {
  ...
  return events.map((e: RawJSON) => {
    ...
    // ADDRESSED: competitor order assumption: this parser infers the current team from competitors[0].homeAway without a team id, so schedules can show the team itself as opponent. EXAMPLE: const self = competitors.find((c) => c.team?.id === teamId); const opponent = competitors.find((c) => c.team?.id !== teamId);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      {items.map((pos: RawJSON, pi: number) => {
        ...
        // ADDRESSED: athletes shape assumed array: non-array depth chart payloads silently render an empty card instead of fallback text. EXAMPLE: const athletes = Array.isArray(pos.athletes) ? pos.athletes : [];
