...
function StatusLabel({ game, league }: { game: BBGame; league: string }) {
  ...
  if (state === "scheduled") {
    ...
    if (game.tipoff) {
      // ADDRESSED: invalid tipoff not guarded: toLocaleString on an invalid Date can render "Invalid Date" in the header. EXAMPLE: const t = Date.parse(game.tipoff); if (Number.isFinite(t)) label = new Date(t).toLocaleString(undefined, { weekday: "short", hour: "2-digit", minute: "2-digit", timeZone: "UTC" }) + " UTC";
...
export default function ScoreHeader({ game, league }: { game: BBGame; league: string }) {
  // ADDRESSED: nested team fields assumed present: game.homeTeam.id/name access will crash for partial game payloads. EXAMPLE: if (!game.homeTeam?.id || !game.awayTeam?.id) return null;
