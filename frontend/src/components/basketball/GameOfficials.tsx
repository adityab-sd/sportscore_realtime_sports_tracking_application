...
export default function GameOfficials({ officials }: { officials: BBOfficial[] }) {
  // ADDRESSED: array prop assumed non-null: officials.length crashes if the API omits the officials collection. EXAMPLE: const safeOfficials = Array.isArray(officials) ? officials : [];
  ...
  return (
    <div style={{ background: "var(--white)", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden" }}>
      ...
      {/* ADDRESSED: index key and order sort can mismatch officials: order changes should not remount names into the wrong row. EXAMPLE: {sorted.map((o) => <div key={`${o.name}:${o.position}`}>...</div>)}. */}
      {sorted.map((o, i) => (
