...
export function AthleteStatTable({ data }: { data: RawJSON }) {
  ...
  // ADDRESSED: stable list keys: category/stat rows use array indexes, so React can attach stale cells if ESPN reorders categories. EXAMPLE: <div key={cat.id ?? cat.name ?? `category-${ci}`}>.
  return (
...
export function AthleteGamelog({ data }: { data: RawJSON }) {
  ...
  // ADDRESSED: missing multiple gamelog categories: only gameLogs[0] renders, so postseason/preseason splits can silently disappear. EXAMPLE: const rows = gameLogs.flatMap((g) => Array.isArray(g.events) ? g.events : []);
