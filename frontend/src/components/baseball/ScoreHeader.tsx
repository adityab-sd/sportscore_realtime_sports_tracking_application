...
import { BBGameDetail } from "@/lib/api/baseball";
import { classifyStatus, inningLabel } from "@/types/baseball";
import TeamLogo from "./TeamLogo";

function StatusLabel({ game }: { game: BBGameDetail }) {
  const state = classifyStatus(game.statusState);
  const dateStr = game.firstPitch && !Number.isNaN(new Date(game.firstPitch).getTime())
    ? new Date(game.firstPitch).toLocaleDateString(undefined, { weekday: "short", month: "short", day: "numeric", year: "numeric" })
    : null;

  if (state === "scheduled") {
    let label = game.status ?? "Scheduled";
    if (game.firstPitch && !Number.isNaN(new Date(game.firstPitch).getTime())) {
      label = new Date(game.firstPitch).toLocaleString(undefined, { weekday: "short", hour: "2-digit", minute: "2-digit" });
    }
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
      ...
      <span style={{ fontSize: 13, fontWeight: 700, color: "#dc2626" }}>{inningLabel(game.inning, game.inningDetail) || "LIVE"}</span>
    </div>
  );
}
  ...
  return (
    <div style={{ background: "var(--white)", border: "1px solid var(--border)", borderRadius: 14, padding: "clamp(14px,3vw,20px)", textAlign: "center" }}>
      <div style={{ fontSize: 11, fontWeight: 500, color: "var(--text-muted)", marginBottom: 12 }}>
        <Link href={`/baseball/league/${league}`} style={{ color: "var(--navy)", textDecoration: "none", fontWeight: 600 }}>{game.competition}</Link>
        {game.venue && <span> · {game.venue}</span>}
      </div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "clamp(8px,3vw,16px)" }}>
        {/* Away first (baseball convention) */}
        <Link href={`/baseball/team/${game.awayTeam.id}?league=${league}`} style={{ flex: 1, textDecoration: "none", minWidth: 0, display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
        ...
        <Link href={`/baseball/team/${game.homeTeam.id}?league=${league}`} style={{ flex: 1, textDecoration: "none", minWidth: 0, display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
