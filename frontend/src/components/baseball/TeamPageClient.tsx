...
import { BBTeam, BBPlayer, BBStandingRow, BBNews, BBGame } from "@/lib/api/baseball";
import { classifyStatus } from "@/types/baseball";
import { LeagueInfo } from "@/types/baseball";
...
// ─── Position grouping (baseball) ──────────────────────────────────────────────
const POS_GROUPS = [
  { keys: ["SP", "RP", "P", "CP"],                 label: "Pitchers" },
  { keys: ["C"],                                   label: "Catchers" },
  { keys: ["1B", "2B", "3B", "SS", "IF"],          label: "Infielders" },
  { keys: ["LF", "CF", "RF", "OF"],                label: "Outfielders" },
  { keys: ["DH"],                                  label: "Designated Hitters" },
];
...
function groupByMonth(games: BBGame[]) {
  ...
  for (const g of games) {
    const key = monthKey(g.firstPitch);
    if (!map.has(key)) map.set(key, { label: monthLabel(g.firstPitch), games: [] });
            ...
            {players.map(p => (
              ...
              >
                <div style={{ display: "flex", flexDirection: "column" }}>
                  ...
                  {p.nationality && <DetailRow label="Bats/Throws">{p.nationality}</DetailRow>}
                  <Link
                    href={`/baseball/player/${p.id}?league=${league}`}
                    style={{ display: "block", marginTop: 14, fontSize: 13, fontWeight: 600, color: "#fff", textDecoration: "none", textAlign: "center", padding: "10px 0", background: accent ?? "var(--navy)", borderRadius: 8 }}
                  >
                    View Player Profile
                  </Link>
                </div>
        ...
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
            {teamLeaders.slice(0, 6).map((l, i) => (
              <Link key={`${l.category}-${i}`} href={l.athleteId ? `/baseball/player/${l.athleteId}?league=${league}` : "#"}
                style={{ background: "var(--white)", border: "1px solid var(--border)", borderRadius: 10, padding: "14px", textDecoration: "none", display: "block", textAlign: "center" }}>
                {l.headshot && (
              ...
              <div style={{ overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
                <table style={{ borderCollapse: "collapse", width: "max-content", minWidth: "100%" }}>
                  <thead>
                    <tr>
                      {cat.stats.map((s, si) => (
                        <th key={`${s.label}-${si}`} style={{ padding: "10px 14px", fontSize: 11, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.4px", borderBottom: "1px solid var(--border)", whiteSpace: "nowrap" }}>{s.label}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    <tr>
                      {cat.stats.map((s, si) => (
                        <td key={`${s.label}-${si}`} style={{ padding: "12px 14px", fontSize: 13, fontWeight: 700, color: "var(--obsidian)", textAlign: "center", whiteSpace: "nowrap" }}>{s.value}</td>
                      ))}
                    </tr>
                  </tbody>
                </table>
...
function FormBadge({ result }: { result: string }) {
  ...
  return (
    <span style={{ width: 24, height: 24, borderRadius: 6, background: color, color: "#fff", fontSize: 11, fontWeight: 800, display: "inline-flex", alignItems: "center", justifyContent: "center" }}>{result}</span>
  );
}
...
export default function TeamPageClient({
  ...
  roster: BBPlayer[];
  standingRow: BBStandingRow | null;
  news: BBNews[];
  seedGames: BBGame[];
  teamStats: { category: string; stats: { label: string; value: string }[] }[];
  teamLeaders: { category: string; name: string; value: string; athleteId: string; headshot: string | null }[];
  ...
  const games = [...seedGames].sort((a, b) => Date.parse(a.firstPitch ?? "0") - Date.parse(b.firstPitch ?? "0"));
  ...
  // Form guide from last 5 finished games (W/L only — no draws in baseball)
  const finished = games
    .filter(g => classifyStatus(g.statusState) === "finished")
    .sort((a, b) => Date.parse(b.firstPitch ?? "0") - Date.parse(a.firstPitch ?? "0"))
    .slice(0, 5);
    ...
    <div>
      ...
      <div style={{ background: bannerColor, position: "relative", overflow: "hidden" }}>
        ...
        <div className="container" style={{ paddingTop: 24, paddingBottom: 0, position: "relative", zIndex: 1 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                ...
                {standingRow && (
                  <span style={{ fontSize: 13, color: "rgba(255,255,255,0.6)" }}>
                    · {standingRow.wins}-{standingRow.losses} · {pct}
                  </span>
                )}
              </div>
            </div>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", borderTop: "1px solid rgba(255,255,255,0.12)" }}>
            ...
            {(tab === "schedule" || tab === "standings" || tab === "stats") && (
      ...
      <div className="container" style={{ paddingTop: 28, paddingBottom: 48 }}>
        {tab === "schedule" && (
          <div>
            {games.length === 0 ? (
              <EmptyState detail={`No games found for ${team.shortName} in ${season}.`} />
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 28 }}>
                {liveG.length > 0 && (
                  <section>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                      {liveG.map(g => <MatchCard key={g.id} game={g} leagueSlug={league} />)}
                    </div>
                  </section>
                )}
                {monthGroups.map(group => (
                  <section key={group.label}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                      {group.games.map(g => <MatchCard key={g.id} game={g} leagueSlug={league} />)}
                    </div>
                  </section>
                ))}
              </div>
        ...
        {tab === "standings" && (
          <div>
            <h2 style={{ fontSize: 16, fontWeight: 800, color: "var(--obsidian)", margin: "0 0 16px" }}>Division Standing</h2>
            {standingRow ? (
              <div style={{ background: "var(--white)", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden" }}>
                <div style={{ display: "grid", gridTemplateColumns: "auto 1fr repeat(5,auto)", gap: 0 }}>
                  ...
                  {[
                    ...
                    standingRow.losses,
                    pct,
                    standingRow.gamesBehind === 0 ? "—" : standingRow.gamesBehind,
                    standingRow.streak ?? "—",
                  ].map((v, i) => (
                ...
                <div style={{ padding: "12px 14px", borderTop: "1px solid var(--border)", textAlign: "right" }}>
                  <Link href={`/baseball/league/${league}`} style={{ fontSize: 12, fontWeight: 600, color: "var(--navy)", textDecoration: "none" }}>View full standings →</Link>
                </div>
              </div>
            ) : (
          ...
          <div>
            ...
            {news.length === 0 ? (
              ...
              <div className="news-grid">
                {news.map(a => <NewsCard key={a.id} article={a} sport="baseball" />)}
              </div>
            )}
          </div>
        )}
