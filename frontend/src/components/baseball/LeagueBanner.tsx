...
import { leagueBySlug } from "@/types/baseball";

interface Props {
  /** Competition name as returned from backend (e.g. "MLB"). */
  name: string;
  /** Resolved league slug for the "View league" link. */
  slug?: string | null;
...
export default function LeagueBanner({ name, slug, showViewLink = true }: Props) {
  ...
  return (
    <div style={{
      ...
      padding: "10px 14px", borderRadius: 10, marginBottom: 12, background: bg,
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
        ...
        <h3 style={{ fontSize: 14, fontWeight: 700, color: "#fff", margin: 0, letterSpacing: "-0.2px", textShadow: "0 1px 2px rgba(0,0,0,0.25)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          {name}
        </h3>
      </div>
      {showViewLink && slug && (
        <Link href={`/baseball/league/${slug}`} style={{ fontSize: 12, fontWeight: 600, color: "#fff", background: "rgba(255,255,255,0.18)", padding: "5px 10px", borderRadius: 6, textDecoration: "none", flexShrink: 0, backdropFilter: "blur(4px)" }}>
          View league →
        </Link>
      )}
    </div>
