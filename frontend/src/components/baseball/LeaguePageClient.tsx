...
import { BBFixture, BBStandingRow, BBNews, BBLeader } from "@/lib/api/baseball";
import { LeagueInfo, classifyStatus } from "@/types/baseball";
...
function pickInitialDate(games: BBFixture[], today: Date): Date {
  ...
  for (const g of games) {
    const t = g.firstPitch ? Date.parse(g.firstPitch) : NaN;
...
export default function LeaguePageClient({
  ...
  const dated = seedGames.filter(g => isSameDay(g.firstPitch, selectedDate));
      ...
      <div style={{ background: `linear-gradient(135deg, ${league.accent} 0%, #0a1628 100%)` }}>
        <div className="container" style={{ paddingTop: 24, paddingBottom: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 20 }}>
            ...
            <div>
              <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(255,255,255,0.5)", textTransform: "uppercase", letterSpacing: "0.6px", marginBottom: 4 }}>Baseball League</div>
      ...
      <div className="container" style={{ paddingTop: 28, paddingBottom: 48 }}>
        ...
        {tab === "news" && (
          <div>
            ...
            {news.length === 0 ? <p style={{ color: "var(--text-muted)", fontSize: 13 }}>No news available.</p> : <div className="news-grid">{news.slice(0, 12).map(a => <NewsCard key={a.id} article={a} sport="baseball" />)}</div>}
          </div>
        )}
