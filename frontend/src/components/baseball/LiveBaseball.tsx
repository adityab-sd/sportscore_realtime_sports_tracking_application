...
import { classifyStatus, leagueName } from "@/types/baseball";
  ...
  return (
    <div style={{ marginBottom: 22 }}>
      ...
      <div className="matches-grid">
        {visible.map(g => <MatchCard key={g.id} game={g} leagueSlug={slug} />)}
      </div>
      {hidden > 0 && (
        <div style={{ marginTop: 8 }}>
          <Link href={`/baseball/league/${slug}`} style={{ fontSize: 12, fontWeight: 500, color: "var(--text-muted)", textDecoration: "none" }}>
            + {hidden} more in {name} →
          </Link>
        </div>
      )}
          ...
          {pills.map(d => {
            ...
            return (
              <button key={key} onClick={() => onSelect(d)}
                ...
                {label.split(" ")[1] && (
                  <span style={{ fontSize: 9, fontWeight: 500, opacity: 0.7, lineHeight: 1.2 }}>{label.split(" ")[1]}</span>
                )}
              </button>
            );
          })}
...
export default function LiveBaseball({ seed = [] }: { seed?: BBGame[] }) {
  ...
  const dated = seed.filter(g => isSameDay(g.firstPitch, selectedDate));
